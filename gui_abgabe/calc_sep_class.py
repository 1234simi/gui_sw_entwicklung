...
# Alle globalen Var
global image_size, maxiter
global calculation_time, calc_mandelbrot_image
global x1, y1, x2, y2


def mandelbrot_maxiter(c, maxiter):
    """
...
# Class Mandelbrot berechnen:
class MandelBrotBerechnen(QThread):
    """
    ...
    def run(self):
        ...
        Am schluss kommt das berechnete Mandelbrot image heraus --> calc_mandelbrot_image
        ...
        maxiter = 500
        tic = time.time()
        calc_mandelbrot_image = np.zeros((image_size, image_size, 3))
        real = np.linspace(x1, x2, calc_mandelbrot_image.shape[1])
        imag = np.linspace(y1, y2, calc_mandelbrot_image.shape[0])

        for col, re in enumerate(real):

            for row, im in enumerate(imag):
                n = mandelbrot_maxiter(complex(re, im), maxiter)
                calc_mandelbrot_image[row, col, :] = smooth_color(n, maxiter)

            # calculate progress in % and set it to the progress-bar
            progress_prozent = (math.ceil(col / image_size * 100))

            self.countChanged.emit(progress_prozent)

            # self.all_progress_bar(progress_prozent)
            # print(progress_)

        tac = time.time()
        calculation_time = (tac - tic)
        print(f'took: {calculation_time} s')
...
class MainWindow(QMainWindow):
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        if local_coor_x <= img_abs_size_x and local_coor_y <= (img_abs_size_y + 38):
            ...
            # self.run()
            self.calculate_Mandelbrot_in_sep_thread()
    ...
    def mandelbrot_parameter(self):
        ...
        image_size = img_original_width
        # Geklickte koordinaten im GUI-Bild
        x = self.point_rel_x
        y = self.point_rel_y
        # Berechnung für Mandelbrot Koordinaten [-2, 2]
        x1_new = x1 + x / image_size * (x2 - x1) - (x2 - x1) / 2.0 / self.value_zoom
        x2_new = x1 + x / image_size * (x2 - x1) + (x2 - x1) / 2.0 / self.value_zoom
        y1_new = y1 + y / image_size * (y2 - y1) - (y2 - y1) / 2.0 / self.value_zoom
        y2_new = y1 + y / image_size * (y2 - y1) + (y2 - y1) / 2.0 / self.value_zoom
        x1 = x1_new
        x2 = x2_new
        y1 = y1_new
        y2 = y2_new
        ...
        print(f'\tMandelbrot_x: {x1} to {x2}')
        print(f'\tMandelbrot_y: {y1} to {y2}')
    ...
    def save_calculated_image(self):
        ...
        ax.imshow(calc_mandelbrot_image, interpolation="bicubic")
        ax.set_xticks([])
        ax.set_yticks([])
        ...
        try:
            ...
            metadata.add_text("calculation time [s]", str(calculation_time))
            metadata.add_text("zoom_faktor", str(self.value_zoom))
            metadata.add_text("Mandelbrot_x_min", str(x1))
            metadata.add_text("Mandelbrot_x_max", str(x2))
            metadata.add_text("Mandelbrot_y_min", str(y1))
            metadata.add_text("Mandelbrot_y_max", str(y2))
    ...
    # def calculate_Mandelbrot_in_sep_thread(self):
    #     # global var_to_get_over
    #     # var_to_get_over = 12
    #     print('calculate_Mandelbrot_in_sep_thread')
    #     self.thread = MandelBrotBerechnen()
    #     self.thread.countChanged.connect(self.onCountChanged)
    #     self.thread.start()
    #
    #
    # def onCountChanged(self, value):
    #     # self.progressBar_calculation.setValue(value)
    #     # self.all_progress_bar(progress_prozent)
    #     # Updating progress bar
    #     self.progressBar.setValue(value)

    def calculate_Mandelbrot_in_sep_thread(self):
        print('calculation_sep_thread')

        self.calc = ClassExtrenal()
...
class ClassExtrenal(QThread):
    ...
    def run(self):
        # print(var_to_get_over)

        rnage = 101
        for x in range(rnage):
            # Calculate Progress
            progress_prozent = (math.ceil((x / rnage) * 100))
            time.sleep(0.1)
